...
#include "../Includes/dlms.h"
#include "../Includes/gxset.h"
#include "../Includes/gxinvoke.h"
#include "../Includes/helpers.h"
#include "../Includes/objectarray.h"
#include "../Includes/ciphering.h"
#include "../Includes/gxget.h"
#include "../Includes/gxkey.h"
#include "../Includes/serverevents.h"
/*TODO:: handle secure element handle*/
    ...
    gxValueEventArg* e)
{
    ...
    if (e->index == 1)
    {
        ...
        if (settings->base.authentication == DLMS_AUTHENTICATION_HIGH_GMAC)
        {
            ...
            readSecret = &tmp;
        } else if (settings->base.authentication == DLMS_AUTHENTICATION_HIGH_ECDSA) {
			
			uint8_t StoC[128];
			int stoc_size = 16 + settings->base.ctoSChallenge.size + settings->base.stoCChallenge.size;
			
			memset(StoC, 0x00, stoc_size);
			memcpy(StoC, settings->base.sourceSystemTitle, 8);
			memcpy(StoC + 8, settings->base.cipher.systemTitle.data, 8);
			memcpy(StoC + 16, settings->base.stoCChallenge.data, settings->base.stoCChallenge.size);
			memcpy(StoC + (16 + settings->base.stoCChallenge.size), settings->base.ctoSChallenge.data, settings->base.ctoSChallenge.size);
			/**TODO:: Secure element integration **/
			/*uint8_t ret = verify_message(StoC, stoc_size, e->parameters.byteArr->data);*/
			
			if(ret) {
				uint8_t CtoS[128];
				uint8_t signature[64];
				
				memset(CtoS, 0x00, stoc_size);
				memset(signature, 0x00, 64);
				memcpy(CtoS, settings->base.cipher.systemTitle.data, 8);
				memcpy(CtoS + 8, settings->base.sourceSystemTitle, 8);
				memcpy(CtoS + 16, settings->base.ctoSChallenge.data, settings->base.ctoSChallenge.size);
				memcpy(CtoS + (16 + settings->base.ctoSChallenge.size), settings->base.stoCChallenge.data, settings->base.stoCChallenge.size);
				/** TODO:: Secure element integration **/
				/*ret = sign_message(CtoS, stoc_size, signature);*/
				
				if(ret) {
					/*e->parameters.byteArr->position -= 5;*/
					BYTE_BUFFER_INIT(&ctos_sig);
					bb_attach(&ctos_sig, tmp3, 0, sizeof(tmp3));
					
					bb_set(&ctos_sig, signature, 64);
					/*bb_attach(&ctos_sig, signature, 64, 64);
					bb_attach(&tmp, settings->base.sourceSystemTitle, sizeof(settings->base.sourceSystemTitle), sizeof(settings->base.sourceSystemTitle));
					
					readSecret = &tmp;*/
				}else{
					set_event(EVENT_NAME_SEC_AUTH_HIGH_ECDSA);
					remove_event(EVENT_NAME_SEC_AUTH_HIGH_ECDSA);
					return DLMS_ERROR_CODE_INVALID_RESPONSE; /* todo SEC_add_set_event*/
		...
		/*TODO: Add handling for DLMS_AUTHENTICATION_HIGH_ECDSA*/
        else
#endif /*DLMS_IGNORE_MALLOC*/
        {
		...
		if (settings->base.authentication == DLMS_AUTHENTICATION_HIGH_GMAC) {
			...
			equal = bb_available(e->parameters.byteArr) != 0 
				&& bb_compare(&bb, e->parameters.byteArr->data + 
				e->parameters.byteArr->position, bb_available(e->parameters.byteArr));
		}
        
        bb_clear(&settings->info.data);
		
		if (settings->base.authentication == DLMS_AUTHENTICATION_HIGH_ECDSA) {
			ret = bb_set2(&settings->info.data, &ctos_sig, 0, ctos_sig.size);
