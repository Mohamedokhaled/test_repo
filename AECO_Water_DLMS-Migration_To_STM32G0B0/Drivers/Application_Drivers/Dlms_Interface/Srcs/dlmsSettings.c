...
#include "../Includes/dlmssettings.h"
    ...
    uint16_t pduBufferSize)
{
    ...
    cl_init(&settings->base, useLogicalNameReferencing,100,145, DLMS_AUTHENTICATION_NONE, NULL, interfaceType);  //Todo Sara : Modify function arguments
    ...
    settings->pushClientAddress = 100;
}
...
void cl_init(
    ...
    DLMS_INTERFACE_TYPE interfaceType)
{
	settings->maxServerPDUSize = settings->maxServerPDUSize;
	settings->maxPduSize = settings->maxPduSize; 
    ...
    if (useLogicalNameReferencing)
    {
        settings->proposedConformance = (DLMS_CONFORMANCE)(
            ...
            DLMS_CONFORMANCE_BLOCK_TRANSFER_WITH_GET_OR_READ |DLMS_CONFORMANCE_BLOCK_TRANSFER_WITH_ACTION |
