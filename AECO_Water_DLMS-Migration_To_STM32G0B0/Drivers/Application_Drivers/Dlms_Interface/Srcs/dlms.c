    ...
    uint16_t index)
{
    ...
    if ((ret = dlms_getHDLCAddress(reply, &target, 0)) != 0)//todo sara change
    {
        return ret;
    }
    if ((ret = dlms_getHDLCAddress(reply, &source, 0)) != 0)//todo sara change
    {
        return ret;
    }
    if (server)
    ...
    gxByteBuffer* reply)
{
	...
	//asamir rest the data pos to handle read assoc view to use correct calculated the length 
	data->position = 0;
    ...
    gxReplyData* data)
{
    ...
    if ((data->moreData & DLMS_DATA_REQUEST_TYPES_FRAME) == 0)
    {
        ...
        {
			if(data->data.data[11] == 0x31 || data->data.data[2] == 0x31) {
				suite = DLMS_SECURITY_SUITE_V1;
			}
    ...
    gxByteBuffer* reply)
{
			...
			|| settings->authentication == DLMS_AUTHENTICATION_HIGH_ECDSA)
    {
        ...
        if (ret == 0)
        {
            //if ((ret = bb_setUInt8(reply, DLMS_SECURITY_AUTHENTICATION_ENCRYPTION + 0x01)) != 0 ||
	...
	//TODO: Add the part for DLMS_AUTHENTICATION_HIGH_ECDSA
#endif //DLMS_IGNORE_HIGH_GMAC
    return ret;
}
