        ...
        else if (result == DLMS_ASSOCIATION_RESULT_ACCEPTED)
        {
            if (settings->base.useLogicalNameReferencing)
            {
                ...
                if (it != NULL)
                {
					...
					//todo asamir move the objects to the current permitted access object 
					//settings->base.objects = it->objectList;
                }
#endif //DLMS_IGNORE_ASSOCIATION_LOGICAL_NAME
            }
        }
    ...
    unsigned char invokeId)
{
    ...
    else
    {
		...
		settings->transaction.command = DLMS_COMMAND_GET_RESPONSE;
    ...
    unsigned char invokeId)
{
	...
	// asamir to reset position , it was generate issue with the ass_view it was copy a part of the pdu not the full pdu 
	data->position = 0;
    params_initLN(&p, &settings->base, invokeId, DLMS_COMMAND_GET_RESPONSE, 2, NULL, data, 0x0, settings->info.encryptedCommand, moreData, !moreData);
