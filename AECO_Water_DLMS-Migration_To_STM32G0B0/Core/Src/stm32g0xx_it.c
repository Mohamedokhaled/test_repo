...
void USART3_4_5_6_IRQHandler(void)
{
  /* USER CODE BEGIN USART3_4_5_6_IRQn 0 */
	...
	uint32_t sr_flags= READ_REG(huart3.Instance->ISR);
	uint32_t cr_bits = READ_REG(huart3.Instance->CR1);
	if (((sr_flags & USART_ISR_RXNE_RXFNE)!= RESET)&&( (cr_bits & USART_CR1_RXNEIE_RXFNEIE)!=RESET))
	{
		huart3.Instance->ISR;
		receive_char =huart3.Instance->RDR;
		rs485_receive_callback(receive_char);
	}
