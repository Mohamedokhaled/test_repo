...
#endif //DLMS_IGNORE_MALLOC
{
    ...
    else if (security == DLMS_SECURITY_AUTHENTICATION_ENCRYPTION || security == 0x31)
    {
        if (encrypt)
        {
            //Encrypt the data.
...
#endif //DLMS_IGNORE_MALLOC
{
	...
	if(*suite == DLMS_SECURITY_SUITE_V0) {
		*security = (DLMS_SECURITY)(ch & 0x30);
	} else if(*suite == DLMS_SECURITY_SUITE_V1) {
		*security = (DLMS_SECURITY)(ch & 0x31);
	}
