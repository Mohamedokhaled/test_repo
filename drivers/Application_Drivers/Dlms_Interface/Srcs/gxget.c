...
int getLNObjects(
    ...
    gxByteBuffer* data)
{
    //asamir make sure that the bb is empty the fill a clean apdu , task assoc_view_readings
	data->size=0;
    uint16_t pduSize = 0;
    ...
    if (!e->transaction)
    {
        ...
        if (!found && 0) // todo asamir skip always the current assoction 
        {
            ++count;
        }
